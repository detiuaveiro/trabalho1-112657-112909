...
// NMec: 112909  Name: José Miguel Tavares Santos
// NMec: 112657  Name: Ricardo Miguel Lopes Carmo
//
//
// Date:
//
...
void ImageStats(Image img, uint8 *min, uint8 *max) { ///
  ...
  *max = 0;
  ...
  // Loop pelos pixels para encontrar o mínimo e o máximo
...
int ImageValidRect(Image img, int x, int y, int w, int h) { ///
  ...
  // Verifica se a área está dentro dos limites da imagem
  int insideWidth = (x >= 0) && (x + w <= img->width);
  int insideHeight = (y >= 0) && (y + h <= img->height);

  // Retorna 1 se a área estiver completamente dentro da imagem, 0 caso
  // contrário
...
Image ImageRotate(Image img) { ///
  ...
  // Cria uma nova imagem para armazenar a versão rotacionada
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      // Calcula a posição do pixel na imagem rotacionada
      int rotatedX = y;
      int rotatedY = width - 1 - x;

      // Define o pixel na imagem rotacionada
      ImageSetPixel(rotatedImg, rotatedX, rotatedY, currentPixel);
    }
  }
...
Image ImageMirror(Image img) { ///
  ...
  // Cria uma nova imagem para armazenar a versão espelhada
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      // Calcula a posição do pixel na versão espelhada
      int mirroredX = width - 1 - x;

      // Define o pixel na imagem espelhada
      ImageSetPixel(mirroredImg, mirroredX, y, currentPixel);
    }
  }
...
Image ImageCrop(Image img, int x, int y, int w, int h) { ///
  ...
  // Cria uma nova imagem para armazenar a região cortada
  ...
  for (int newY = 0; newY < h; newY++) {
    for (int newX = 0; newX < w; newX++) {
      // Obtém o valor do pixel da imagem original
      uint8 currentPixel = ImageGetPixel(img, x + newX, y + newY);

      // Define o pixel na imagem cortada
      ImageSetPixel(croppedImg, newX, newY, currentPixel);
    }
  }
...
void ImagePaste(Image img1, int x, int y, Image img2) { ///
  ...
  for (int newY = 0; newY < height2; newY++) {
    for (int newX = 0; newX < width2; newX++) {
      // Obtém o valor do pixel da imagem menor
      uint8 currentPixel = ImageGetPixel(img2, newX, newY);

      // Define o pixel na posição correspondente da imagem maior
      ImageSetPixel(img1, x + newX, y + newY, currentPixel);
    }
  }
}
...
void ImageBlend(Image img1, int x, int y, Image img2, double alpha) { ///
  ...
  int width2 = ImageWidth(img2);
  int height2 = ImageHeight(img2);
  uint8 maxval = ImageMaxval(img1);

  for (int newY = 0; newY < height2; newY++) {
    for (int newX = 0; newX < width2; newX++) {
      // Obtém o valor do pixel das duas imagens
      uint8 pixel1 = ImageGetPixel(img1, x + newX, y + newY);
      uint8 pixel2 = ImageGetPixel(img2, newX, newY);

      // Calcula o valor do pixel
      double blendedValue = alpha * pixel2 + (1.0 - alpha) * pixel1 + 0.5;

      // Satura os valores para garantir que estejam dentro do intervalo [0,
      // maxval]
      if (blendedValue < 0.0) {
        blendedValue = 0;
      } else if (blendedValue > maxval) {
        blendedValue = maxval;
      } else {
        blendedValue = (uint8)blendedValue;
      }

      // Define o pixel na posição correspondente da imagem maior
      ImageSetPixel(img1, x + newX, y + newY, (uint8)blendedValue);
    }
  }
}
...
int ImageMatchSubImage(Image img1, int x, int y, Image img2) { ///
  ...
  for (int j = 0; j < height2; j++) {
    for (int i = 0; i < width2; i++) {
      ...
      if (pixel1 != pixel2) {
        // Os pixels não coincidem, portanto, não há correspondência
        return 0;
      }
    }
  }

  // Todos os pixels coincidem, há uma correspondência
  return 1;
}
...
void ImageBlur(Image img, int dx, int dy) { ///
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      // Calcula a média e define o valor do pixel na imagem temporária
      uint8 average = (double)((sum + count / 2) / count);
